...
const openScore = async (row) => {
  Object.assign(scoreForm, { id: null, submitId: row.id, homeworkId: row.homeworkId, xingming: row.xingming, xuehao: row.xuehao, userid: row.userid, homeworkTitle: row.homeworkTitle, content: row.content, fujian: row.fujian, score: 0, pingyuContent: '' })
  if (row.zhuangtai === '已批改') {
    try {
      const res = await request.get('/homework/score/page', { params: { homeworkId: row.homeworkId, xingming: row.xingming, limit: 1 } })
      const scoreData = res.data?.list?.[0]
      if (scoreData) {
        scoreForm.id = scoreData.id
        scoreForm.score = scoreData.score || 0
        scoreForm.pingyuContent = scoreData.pingyuContent || ''
      }
    } catch (e) {}
  }
