  ...
  <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑' : '布置作业'" width="600px">
    <el-form :model="form" label-width="90px">
      ...
      <el-form-item label="截止日期">
        <el-date-picker v-model="form.jiezhishijian" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" style="width: 100%;" />
      </el-form-item>
...
const loadData = async () => {
  const res = await request.get('/homework/page', { params: { page: page.value, limit: 10, faburenxingming: teacherName, ...search } })
  list.value = res.data?.list || []
  total.value = res.data?.totalCount || 0
}
onMounted(async () => {
  const [t, b] = await Promise.all([
    request.get('/courseType/list'),
    request.get('/student/banjiList')
  ])
  types.value = t.data || []
...
const openDialog = (row) => {
  isEdit.value = !!row
  Object.assign(form, row || { id: null, title: '', coursetype: '', banji: '', jiezhishijian: '', content: '', faburenxingming: teacherName })
  dialogVisible.value = true
}
const handleSave = async () => {
  form.faburenxingming = teacherName
  if (!form.banji) form.banji = ''
  if (isEdit.value) await request.put('/homework/update', form)
  else await request.post('/homework/save', form)
...
const handleDelete = async (row) => {
  ...
  await request.delete('/homework/delete', { data: [row.id] })
