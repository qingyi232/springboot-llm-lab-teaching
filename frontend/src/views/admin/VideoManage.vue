  ...
  <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑' : '添加'" width="600px">
    <el-form :model="form" label-width="80px">
      ...
      <el-form-item label="视频文件">
        <el-upload class="video-uploader" action="/api/file/upload" :headers="uploadHeaders" :show-file-list="false" :on-success="handleVideoSuccess" :on-progress="handleVideoProgress" :before-upload="beforeVideoUpload" accept="video/*">
          <div v-if="form.videourl" class="video-uploaded">
...
const beforeVideoUpload = (file) => {
  ...
  videoUploading.value = true; videoProgress.value = 0
  return true
}
...
const handleVideoSuccess = (res) => {
  videoUploading.value = false; videoProgress.value = 100
  if (res.code === 0) { form.videourl = res.url; videoFileName.value = res.fileName || '视频已上传'; ElMessage.success('视频上传成功') }
  else { ElMessage.error(res.msg || '上传失败') }
}
const handleVideoProgress = (evt) => { videoProgress.value = Math.round(evt.percent || 0) }
const loadData = async () => { const res = await request.get('/video/page', { params: { page: page.value, limit: 10, ...search } }); list.value = res.data?.list || []; total.value = res.data?.totalCount || 0 }
onMounted(async () => { const t = await request.get('/courseType/list'); types.value = t.data || []; loadData() })
const openDialog = (row) => {
  isEdit.value = !!row
  Object.assign(form, row || { id: null, title: '', coursetype: '', tupian: '', videourl: '', content: '', faburenxingming: '' })
  videoFileName.value = ''; videoUploading.value = false; videoProgress.value = 0
  dialogVisible.value = true
}
const handleSave = async () => { if (isEdit.value) await request.put('/video/update', form); else await request.post('/video/save', form); ElMessage.success('保存成功'); dialogVisible.value = false; loadData() }
...
.video-uploader :deep(.el-upload) { border: 1px dashed #d9d9d9; border-radius: 6px; cursor: pointer; padding: 12px 20px; width: 100%; }
