<template>
  <CrudTable :data="list" :total="total" v-model:page="page" @search="loadData" @add="openDialog()" @edit="openDialog($event)" @del="handleDelete($event)" @page-change="loadData">
    <template #search><el-input v-model="search.labName" placeholder="实验室名称" clearable style="width: 200px;" /><el-select v-model="search.shiyanshileixing" placeholder="类型" clearable style="width: 160px;"><el-option v-for="t in types" :key="t.typename" :label="t.typename" :value="t.typename" /></el-select></template>
    <el-table-column prop="labId" label="编号" width="100" /><el-table-column prop="labName" label="名称" width="180" /><el-table-column prop="shiyanshileixing" label="类型" width="140" /><el-table-column prop="weizhi" label="位置" width="160" /><el-table-column prop="rongna" label="容纳" width="80" /><el-table-column prop="zhuangtai" label="状态" width="80"><template #default="{ row }"><el-tag :type="row.zhuangtai === '空闲' ? 'success' : 'warning'" size="small">{{ row.zhuangtai }}</el-tag></template></el-table-column><el-table-column prop="guanliyuan" label="管理员" width="100" />
  </CrudTable>
  <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑' : '添加'" width="600px">
    <el-form :model="form" label-width="100px">
      <el-row :gutter="16"><el-col :span="12"><el-form-item label="实验室编号"><el-input v-model="form.labId" /></el-form-item></el-col><el-col :span="12"><el-form-item label="名称"><el-input v-model="form.labName" /></el-form-item></el-col></el-row>
...
const list = ref([]); const types = ref([]); const total = ref(0); const page = ref(1); const dialogVisible = ref(false); const isEdit = ref(false); const search = reactive({ labName: '', shiyanshileixing: '' }); const form = reactive({})
...
const handleDelete = async (row) => { await ElMessageBox.confirm('确定删除？'); await request.delete('/labInfo/delete', { data: [row.id] }); ElMessage.success('已删除'); loadData() }
</script>
