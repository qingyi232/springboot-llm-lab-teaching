...
const loadData = async () => { const res = await request.get('/courseType/page', { params: { page: page.value, limit: 10, ...search } }); list.value = res.data?.list || []; total.value = res.data?.totalCount || 0 }
onMounted(() => loadData())
const openDialog = (row) => { isEdit.value = !!row; Object.assign(form, row || { id: null, typename: '' }); dialogVisible.value = true }
const handleSave = async () => { if (isEdit.value) await request.put('/courseType/update', form); else await request.post('/courseType/save', form); ElMessage.success('保存成功'); dialogVisible.value = false; loadData() }
const handleDelete = async (row) => { await ElMessageBox.confirm('确定删除？'); await request.delete('/courseType/delete', { data: [row.id] }); ElMessage.success('已删除'); loadData() }
</script>
