...
onMounted(async () => {
  try {
    const [cm, v, hw, hs] = await Promise.all([
      ...
      request.get('/homework/submit/page', { params: { page: 1, limit: 100, zhuangtai: '已提交', faburenxingming: teacherName } }),
    ])
    ...
    pendingSubmits.value = hs.data?.list || []
    stats.value[3].value = hs.data?.totalCount || 0
  } catch (e) {}
})
</script>
