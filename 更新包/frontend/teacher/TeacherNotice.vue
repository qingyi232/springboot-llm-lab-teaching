<template>
  ...
  <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑' : '发布公告'" width="600px">
    <el-form :model="form" label-width="80px">
      ...
      <el-form-item label="分类"><el-select v-model="form.typename" style="width: 100%;" allow-create filterable><el-option label="教务通知" value="教务通知" /><el-option label="设备通知" value="设备通知" /><el-option label="安全通知" value="安全通知" /></el-select></el-form-item>
...
const teacherName = userStore.userInfo.jiaoshixingming || userStore.userInfo.username
...
const loadData = async () => { const res = await request.get('/notice/page', { params: { page: page.value, limit: 10, name: teacherName, ...search } }); list.value = res.data?.list || []; total.value = res.data?.totalCount || 0 }
onMounted(() => loadData())
const openDialog = (row) => { isEdit.value = !!row; Object.assign(form, row || { id: null, title: '', typename: '教务通知', introduction: '', name: teacherName }); dialogVisible.value = true }
const handleSave = async () => { form.name = teacherName; if (isEdit.value) await request.put('/notice/update', form); else await request.post('/notice/save', form); ElMessage.success('保存成功'); dialogVisible.value = false; loadData() }
