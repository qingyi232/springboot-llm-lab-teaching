...
const loadData = async () => { const res = await request.get('/courseMaterial/page', { params: { page: page.value, limit: 10, faburenxingming: teacherName, ...search } }); list.value = res.data?.list || []; total.value = res.data?.totalCount || 0 }
onMounted(async () => { const t = await request.get('/courseType/list'); types.value = t.data || []; loadData() })
const openDialog = (row) => { isEdit.value = !!row; Object.assign(form, row || { id: null, title: '', coursetype: '', tupian: '', content: '', faburenxingming: teacherName }); dialogVisible.value = true }
const handleSave = async () => { form.faburenxingming = teacherName; if (isEdit.value) await request.put('/courseMaterial/update', form); else await request.post('/courseMaterial/save', form); ElMessage.success('保存成功'); dialogVisible.value = false; loadData() }
